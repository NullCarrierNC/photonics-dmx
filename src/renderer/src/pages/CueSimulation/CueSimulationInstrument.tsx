...
const DRUM_NOTE_BUTTON_CLASS = 'px-3 py-1 text-xs rounded hover:opacity-80 disabled:opacity-50 disabled:cursor-not-allowed';
const INSTRUMENT_NOTE_BUTTON_CLASS = 'px-3 py-1 text-xs rounded hover:opacity-80 disabled:opacity-50 disabled:cursor-not-allowed';

function drumNoteClass(note: string): string {
  const base = `${DRUM_NOTE_BUTTON_CLASS} `;
  if (note === DrumNoteType.GreenDrum || note === DrumNoteType.GreenCymbal) return base + 'bg-green-500 text-white';
  if (note === DrumNoteType.RedDrum) return base + 'bg-red-500 text-white';
  if (note === DrumNoteType.YellowDrum || note === DrumNoteType.YellowCymbal) return base + 'bg-yellow-500 text-white';
  if (note === DrumNoteType.BlueDrum || note === DrumNoteType.BlueCymbal) return base + 'bg-blue-500 text-white';
