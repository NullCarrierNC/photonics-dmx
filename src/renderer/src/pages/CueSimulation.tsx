...
const CueSimulation: React.FC = () => {
  ...
  useEffect(() => {
    const loadSavedEffect = async () => {
      ...
      const checkForEffects = async (retries = 10) => {
        try {
          const availableEffects = await window.electron.ipcRenderer.invoke(LIGHT.GET_AVAILABLE_CUES, selectedGroupId);
  ...
  useEffect(() => {
    if (selectedGroupId && !isSettingActiveGroup.current) {
      ...
      window.electron.ipcRenderer.invoke(LIGHT.SET_ACTIVE_CUE_GROUPS, [selectedGroupId])
        .then(result => {
          if (result.success) {
  ...
  const ensureActiveGroupMatches = useCallback(async () => {
    if (selectedGroupId && isFullyInitialized.current && !isSettingActiveGroup.current) {
      try {
        isSettingActiveGroup.current = true;
        const result = await window.electron.ipcRenderer.invoke(LIGHT.SET_ACTIVE_CUE_GROUPS, [selectedGroupId]);
        if (result.success) {
          activeGroupsSelectorRef.current?.refreshActiveGroups();
