...
import { createDefaultActionTiming, type ActionNode, type AudioEventNode, type YargEventNode, type LogicNode } from '../../../../../photonics-dmx/cues/types/nodeCueTypes';
...
export function useEdgeManagement({
  ...
  const isValidNodeConnection = useCallback((sourceId?: string | null, targetId?: string | null) => {
    ...
    if (targetNode.data.kind === 'event-listener' || targetNode.data.kind === 'effect-listener') return false;
    const validTargets = ['action', 'logic', 'event-raiser', 'effect-raiser'];
    if (sourceNode.data.kind === 'event' && validTargets.includes(targetNode.data.kind)) return true;
    if (sourceNode.data.kind === 'logic' && validTargets.includes(targetNode.data.kind)) return true;
    if (sourceNode.data.kind === 'action' && validTargets.includes(targetNode.data.kind)) return true;
    if (sourceNode.data.kind === 'event-raiser' && validTargets.includes(targetNode.data.kind)) return true;
    if (sourceNode.data.kind === 'effect-raiser' && validTargets.includes(targetNode.data.kind)) return true;
    if (sourceNode.data.kind === 'event-listener' && validTargets.includes(targetNode.data.kind)) return true;
    if (sourceNode.data.kind === 'effect-listener' && validTargets.includes(targetNode.data.kind)) return true;
    return false;
  }, [nodes]);
        ...
        if (sourceNode.data.kind === 'event') {
          ...
          const inheritedWaitForCondition =
            sourceEvent.eventType === 'cue-started' || sourceEvent.eventType === 'cue-called'
              ? 'none'
              : (sourceEvent.eventType as 'none' | 'delay' | 'beat' | 'measure' | 'half-beat' | 'keyframe');
          targetAction.timing = {
