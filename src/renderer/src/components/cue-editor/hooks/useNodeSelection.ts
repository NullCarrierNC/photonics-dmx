...
import type { EffectRaiserNode, EffectEventListenerNode } from '../../../../../photonics-dmx/cues/types/nodeCueTypes';
...
export function useNodeSelection({
  ...
  const handlePaneContextMenu = useCallback((event: React.MouseEvent) => {
    ...
    setPaneContextMenu({ x: adjustedX, y: adjustedY, flowX: flowPosition.x, flowY: flowPosition.y });
  }, [reactFlowInstance, flowWrapperRef]);

  const updateSelectedNode = useCallback(<T extends YargEventNode | AudioEventNode | ActionNode | LogicNode | EventRaiserNode | EventListenerNode | EffectRaiserNode | EffectEventListenerNode | NotesNode>(updates: Partial<T>) => {
