...
type VariableRegistryProps = {
  ...
  onVariablesChange: (
    groupVars: VariableDefinition[],
    cueVars: VariableDefinition[]
  ) => void;
...
const VariableRegistry: React.FC<VariableRegistryProps> = ({
  ...
  const effectVariables =
    isEffectMode && currentEffect ? currentEffect.variables ?? [] : [];
  ...
  const handleSave = () => {
    if (
      !formData.name ||
      !formData.type ||
      formData.initialValue === undefined
    ) {
