...
function getInitialValueInput(
  ...
  const selectedEventValue =
    typeof value === 'string' && eventOptionValues.includes(value)
      ? value
      : defaultEventValue;
...
const VariableFormDialog: React.FC<VariableFormDialogProps> = ({
  ...
  const handleTypeChange = (newType: VariableType) => {
    ...
    if (newType === 'boolean') newValue = false;
      ...
      newValue = getDefaultEventOption(activeMode)?.value ?? '';
          ...
          {isEffectMode && (
            <label className="flex items-center gap-2 font-medium text-sm">
              <input
                ...
                onChange={e =>
                  onFormDataChange({ ...formData, isParameter: e.target.checked })
                }
                className="rounded"
              />
