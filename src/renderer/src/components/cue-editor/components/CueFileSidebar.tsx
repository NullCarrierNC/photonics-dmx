...
import type { NodeCueMode, YargNodeCueDefinition, AudioNodeCueDefinition, NodeCueFile, EffectFile } from '../../../../../photonics-dmx/cues/types/nodeCueTypes';
      ...
      <div className="space-y-2 overflow-y-auto max-h-[180px]">
        ...
        {displayFileList.map(file => (
          <button
            ...
            onClick={() => isEffectMode ? onSelectEffectFile(file as EffectFileSummary) : onSelectFile(file as NodeCueFileSummary)}
          >
            <div className="font-semibold truncate">{file.groupName}</div>
            <div className="text-[10px] text-gray-500 truncate">{file.path}</div>
            <div className="text-[10px] text-gray-500">{file.cueCount} {itemCountLabel}</div>
          </button>
        ))}
      </div>
          ...
          {items.map(item => (
            <div key={item.id} className="flex items-center gap-2">
              <button
                ...
                onClick={() => onSelectCue(item as any)}
              >
                {item.name}
              </button>
              <button
