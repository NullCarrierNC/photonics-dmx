...
import { useWarningNodesContext } from '../../context/WarningNodesContext'
 ...
 * from context (in that precedence); then children (node content and Handles). Use as the root
 * element of each node component.
 */
const FlowNodeFrame: React.FC<Props> = ({ id, className, style, children }) => {
  ...
  const warningNodeIds = useWarningNodesContext()
  const isActive = activeNodeIds.has(id)
  const isError = errorNodeIds.has(id)
  const isWarning = warningNodeIds.has(id)
  const highlightStyles = isError
    ? ERROR_STYLES
    : isWarning
      ? WARNING_STYLES
      : isActive
