...
const LogicNodeComponent: React.FC<NodeProps<EditorNodeData>> = ({ id, data, selected }) => {
  ...
  const renderDetails = (): React.ReactNode => {
    ...
    if (logicType === 'debugger') {
      ...
      return (
        <>
          <div>Message: <Mono>{messageText || '(empty)'}</Mono></div>
          <div>Vars: <Mono>{variables.length > 0 ? variables.join(', ') : 'none'}</Mono></div>
        </>
      );
    }
    return logicType;
  ...
  return (
    <div className={`px-3 py-2 rounded-lg border-2 ${nodeStyles} ${selectedStyles} ${activeStyles}`}>
