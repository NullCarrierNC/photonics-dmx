...
const EventListenerNode: React.FC<NodeProps<EditorNodeData>> = ({ id, data, selected }) => {
  ...
  return (
    <div className={`px-3 py-2 rounded-lg border-2 border-purple-400 bg-purple-50 dark:bg-purple-900/40 text-xs shadow-sm min-w-[140px] ${selectedStyles} ${activeStyles}`}>
      <div className="flex items-center gap-1 font-semibold text-purple-800 dark:text-purple-100">
        ...
        <span>{listenerPayload.eventName ? <>Listen · <span style={FONT_COURIER_NEW}>{eventName}</span></> : 'Listen Event'}</span>
      </div>
