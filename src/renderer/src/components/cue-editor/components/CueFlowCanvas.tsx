      ...
      >
          ...
          <>
            ...
            {addEventListenerNode && (
              <button
                ...
                onClick={() => handlePaneMenuClick(() => addEventListenerNode({ x: paneContextMenu.flowX, y: paneContextMenu.flowY }))}
              >
            ...
            <button
              ...
              onClick={() => handlePaneMenuClick(() => addEventNode(getDefaultEventOption(activeMode), { x: paneContextMenu.flowX, y: paneContextMenu.flowY }))}
            >
              System Event
            </button>
          </>
        ...
        {editorMode === 'effect' && addEffectListenerNode && (
          <>
            ...
            <button
              ...
              onClick={() => handlePaneMenuClick(() => addEffectListenerNode({ x: paneContextMenu.flowX, y: paneContextMenu.flowY }))}
            >
              Effect Listener
            </button>
          </>
        ...
        {[...NODE_EFFECT_TYPES].sort().map(effectType => (
          <button
            ...
            onClick={() => handlePaneMenuClick(() => addActionNode(effectType, { x: paneContextMenu.flowX, y: paneContextMenu.flowY }))}
          >
            {effectType}
          </button>
        ))}
        ...
        {(['variable', 'math', 'conditional', 'cue-data', 'config-data', 'lights-from-index', 'for-loop', 'while-loop', 'array-length', 'reverse-lights', 'create-pairs', 'concat-lights', 'delay'] as LogicNode['logicType'][]).sort().map(logicType => (
          <button
            ...
            onClick={() => handlePaneMenuClick(() => addLogicNode(logicType, { x: paneContextMenu.flowX, y: paneContextMenu.flowY }))}
          >
        ...
        {addEventRaiserNode && (
          <>
            ...
            <button
              className="block w-full text-left px-3 py-1 text-purple-800 dark:text-purple-100 bg-purple-50 dark:bg-purple-900/40 hover:bg-purple-100 dark:hover:bg-purple-900/60"
              onClick={() => handlePaneMenuClick(() => addEventRaiserNode({ x: paneContextMenu.flowX, y: paneContextMenu.flowY }))}
            >
              Custom Event Raiser
            </button>
          </>
