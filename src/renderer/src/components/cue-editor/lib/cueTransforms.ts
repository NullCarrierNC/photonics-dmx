...
const updateDocumentFromFlow = (
  ...
  const validEdges = edges.filter(edge => {
    ...
    const validSourceKinds = ['event', 'action', 'logic', 'event-raiser', 'event-listener', 'effect-raiser'];
  ...
  const updatedCues = (editorDoc.file as NodeCueFile).cues.map(cue => (cue.id === updatedCue.id ? updatedCue : cue));
  return {
...
const effectModeOf = (effect: EffectDefinition): NodeCueMode =>
  effect.mode as NodeCueMode;
...
const updateEffectDocumentFromFlow = (
  ...
  const validEdges = edges.filter(edge => {
    ...
    const validSourceKinds = ['event', 'action', 'logic', 'event-raiser', 'event-listener', 'effect-listener'];
