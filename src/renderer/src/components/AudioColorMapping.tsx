...
const DEFAULT_RANGES = [
  {
    ...
    maxHz: 220,
    color: 'red' as Color,
    brightness: 'medium' as Brightness,
    sensitivity: 1.0,
  },
  {
    id: 'range2',
...
const AudioColorMapping: React.FC = () => {
  ...
  const [localSensitivityValues, setLocalSensitivityValues] = useState<Map<string, number>>(
    new Map(),
  )
    ...
    const loadConfig = async () => {
      try {
        ...
        if (
          ...
          config.frequencyBands.ranges.length > 0
        ) {
  ...
  const saveConfig = useCallback(
    async (updatedRanges: FrequencyRange[], overrideBandCount?: 3 | 4 | 5) => {
      if (isSaving) return
  ...
  const handleRangeChange = useCallback(
    (rangeId: string, minHz: number, maxHz: number) => {
  ...
  const handleBrightnessChange = useCallback(
    (rangeId: string, brightness: Brightness) => {
      const updatedRanges = ranges.map((range) =>
  ...
  const handleSensitivityChange = useCallback(
    async (rangeId: string, sensitivity: number) => {
      const updatedRanges = ranges.map((range) =>
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Configure frequency ranges, colours, and brightness levels for audio-reactive lighting.
        Overlapping ranges will blend colours additively.
      </p>
