...
type LightChannelsCardProps = {
  light: DmxFixture
  dmxValues: Record<number, number>
  sharedChannels: Set<number>
}

/** A fixture's card only redraws when its own addresses move; see {@link fixtureDmxValuesEqual}. */
const cardPropsEqual = (a: LightChannelsCardProps, b: LightChannelsCardProps): boolean =>
  a.light === b.light &&
  a.sharedChannels === b.sharedChannels &&
  fixtureDmxValuesEqual(a.light, a.dmxValues, b.dmxValues)

/** One light's channels and their live values. */
const LightChannelsCard = React.memo(function LightChannelsCard({
  light,
  dmxValues,
  sharedChannels,
}: LightChannelsCardProps) {
  // Sort order and extra-channel labels depend only on the fixture, so they are derived once per
  // fixture and not per DMX frame.
  const sortedEntries = useMemo(
    () => sortBaseChannelEntries(Object.entries(light.channels)),
    [light],
  )
  const extraLabels = useMemo(
    () => (light.extraChannels ?? []).map((_, i) => extraChannelDisplayLabel(light, i)),
    [light],
  )
