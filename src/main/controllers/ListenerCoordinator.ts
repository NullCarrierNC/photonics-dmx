...
export class ListenerCoordinator {
  ...
  public enableYarg(isInitialized: boolean, initAsync: () => Promise<void>): void {
    if (!isInitialized) {
      ...
      initAsync().then(() => this.enableYargInternal()).catch(error => {
  ...
  public async disableYarg(): Promise<void> {
    ...
    if (effectsController) {
      try {
        ...
        console.log('ListenerCoordinator: Cleared all running effects and initiated blackout when disabling YARG');
      } catch (error) {
  ...
  public async enableRb3(isInitialized: boolean, initAsync: () => Promise<void>): Promise<void> {
    if (!isInitialized) {
      ...
      initAsync().then(() => this.enableRb3Internal()).catch(error => {
        console.error("Error during initialization:", error);
      });
      return;
    }
  ...
  public async enableRb3Internal(): Promise<void> {
    ...
    this.processorManager = new ProcessorManager(dmxLightManager, effectsController, { mode: 'direct' });
  ...
  public async disableRb3(): Promise<void> {
    ...
    if (effectsController) {
      try {
        effectsController.removeAllEffects();
        await effectsController.blackout(0);
        console.log('ListenerCoordinator: Cleared all running effects and initiated blackout when disabling RB3');
      } catch (error) {
