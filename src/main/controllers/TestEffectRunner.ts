...
export interface TestEffectRunnerContext {
  ...
  createCueHandler: (dmxLightManager: DmxLightManager, effectsController: ILightingController, debouncePeriod: number) => YargCueHandler;
...
export class TestEffectRunner {
  ...
  public startTestEffect(effectId: string, venueSize?: 'NoVenue' | 'Small' | 'Large', bpm?: number): void {
    console.log(`TestEffectRunner.startTestEffect effectId: ${effectId}, venueSize: ${venueSize}, BPM: ${bpm}`);
    ...
    this.ctx.ensureInitialized().then(() => {
      this.startInternal();
    }).catch((error: unknown) => {
