...
export class BeatSpectrumMorphCue implements IAudioCue {
  ...
  cueType = AudioCueType.BeatSpectrumMorph;
  description = 'On each beat, moves the entire rig toward the dominant spectrum colour, then drifts back between beats';
  ...
  async execute(
    ...
    const rangeColor = getRangeAndColor(data.config, dominant.bandIndex, dominant.intensity, 'add', 1.4);
    if (!rangeColor) {
      return;
    }
