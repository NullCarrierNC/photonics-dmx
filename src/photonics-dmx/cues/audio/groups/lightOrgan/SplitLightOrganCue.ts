...
export class SplitLightOrganCue implements IAudioCue {
  ...
  cueType = BuiltInAudioCues.SplitLightOrgan
  description =
    'Low bands sit on the front line while upper bands mirror on the back for a two-plane organ'
  ...
  async execute(
    ...
    const applySegments = (
      ...
      segments.forEach((segment, localIndex) => {
        ...
        const bandIndex =
          bandIndices[startIndex + localIndex] ?? bandIndices[bandIndices.length - 1]
