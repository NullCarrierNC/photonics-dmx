...
export class BassSnareRippleCue implements IAudioCue {
  ...
  cueType = AudioCueType.BassSnareRipple;
  description = 'Bass pushes a warm ripple across the front wash while mids snap the rear lights for snare hits';
  ...
  async execute(
    ...
    if (frontLights.length > 0 && bassLevel > bassThreshold) {
      ...
      bassColor.intensity = Math.min(255, Math.floor(bassColor.intensity * (0.6 + adjustedBassLevel)));
    ...
    if (backLights.length > 0 && snareLevel > snareThreshold) {
      ...
      snareColor.intensity = Math.min(255, Math.floor(snareColor.intensity * (0.5 + adjustedSnareLevel)));
