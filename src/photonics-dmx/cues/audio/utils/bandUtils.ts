...
export const getBandValue = (
  bands: AudioLightingData['frequencyBands'],
  bandIndex: number,
): number => {
  switch (bandIndex) {
    case 0:
...
export const getAverageBandIntensity = (
  ...
  const total = bandIndices.reduce(
    (sum, index) => sum + getBandValue(audioData.frequencyBands, index),
    0,
  )
...
export const getOrganColorByIndex = (index: number): Color => {
  ...
  const normalizedIndex =
    ((index % ORGAN_COLOR_SEQUENCE.length) + ORGAN_COLOR_SEQUENCE.length) %
    ORGAN_COLOR_SEQUENCE.length
