...
export class CoolAutomaticCue implements ICue {
  ...
  description = 'Set each light to a random colour of red or blue. On the beat randomly invert one of the lights with a flash.';
  ...
  async execute(_parameters: CueData, sequencer: ILightingController, lightManager: DmxLightManager): Promise<void> {
    ...
    if (isNewSession) {
      ...
      allLights.forEach((light) => {
        const randomColor = randomBetween(0, 1) === 0 ? 'red' : 'blue';
        lightStates[light.id] = randomColor;
        
        const color = randomColor === 'red' ? red : blue;
      ...
      const baseEffect: Effect = {
        ...
        description: 'Set all lights to random red or blue base colors',
    ...
    const invertedColor = currentState === 'red' ? blueHigh : redHigh;
