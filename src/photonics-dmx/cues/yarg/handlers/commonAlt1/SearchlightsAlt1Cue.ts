...
export class SearchlightsAlt1Cue implements INetCue {
  ...
  async execute(
    ...
    const sortedFrontLights = frontLights.sort(
      (a: TrackedLight, b: TrackedLight) => a.position - b.position,
    )
    const sortedBackLights = backLights.sort(
      (a: TrackedLight, b: TrackedLight) => b.position - a.position,
    )
    ...
    console.log(
      `[SearchlightsCue] Fading in light ${newLight.id} to ${brightness} brightness over ${twoBeatsDuration}ms`,
    )
