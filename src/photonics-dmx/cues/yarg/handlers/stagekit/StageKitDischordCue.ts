...
export class StageKitDischordCue implements INetCue {
  ...
  description =
    'Yellow clockwise on beat, green dual-mode (spinning/solid) on measure, blue alternating patterns on keyframe, red flash on red drum.'
  ...
  async execute(
      ...
      for (let lightIndex = 0; lightIndex < allLights.length; lightIndex++) {
        ...
        if (stepsUntilGreen > 0) {
          greenTransitions.push({
            ...
            waitUntilCondition: 'beat',
            waitUntilTime: 0,
            waitUntilConditionCount: Math.floor(
              (stepsUntilGreen * beatsPerCycle) / allLights.length,
            ),
          })
        }
        ...
        if (stepsAfterGreen > 0) {
          greenTransitions.push({
            ...
            waitUntilTime: 0,
            waitUntilConditionCount: Math.floor(
              (stepsAfterGreen * beatsPerCycle) / allLights.length,
            ),
          })
        }
      }
    ...
    if (this.isFirstExecution) {
      ...
      controller.setEffect('dischord-blue', blueEffect)
