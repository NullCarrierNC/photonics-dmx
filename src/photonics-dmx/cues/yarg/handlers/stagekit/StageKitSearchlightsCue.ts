...
export class StageKitSearchlightsCue implements INetCue {
  ...
  description =
    'Dual counter-clockwise patterns: Yellow (0→7→6→5→4→3→2→1) and Red offset by 1 position. 0.5 cycles per beat timing. For <8 lights: Yellow only.'
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- light position type from mapper
  private createYellowCounterClockwiseEffect(allLights: any[], lightDuration: number): Effect {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- light position type from mapper
  private createRedCounterClockwiseEffect(allLights: any[], lightDuration: number): Effect {
