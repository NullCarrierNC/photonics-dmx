...
export class ChorusCue implements INetCue {
  ...
  description =
    'Alternating randomly between Amber/Purple or Yellow/Red colors on all lights with timing based on song BPM'
  ...
  async execute(
    ...
    for (let i = 0; i < num; i++) {
      const color =
        flip === 1
          ? set1[randomBetween(0, set1.length - 1)]
          : set2[randomBetween(0, set2.length - 1)]
