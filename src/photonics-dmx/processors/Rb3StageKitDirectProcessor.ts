...
export class Rb3StageKitDirectProcessor extends EventEmitter {
  ...
  private pendingUpdates: Map<number, { colors: Set<string>; timeout: NodeJS.Timeout | null }> =
    new Map() // DMX light index -> pending update
  private readonly ACCUMULATION_DELAY_MS = 5

  // Track active strobe effects for cleanup
  private activeStrobeEffects: Map<
    string,
    {
  ...
  constructor(
    ...
    if (numLights < 4) {
      ...
      throw new Error(
        `StageKit requires at least 4 DMX lights, but only ${numLights} are configured`,
      )
    } else if (numLights < 8) {
