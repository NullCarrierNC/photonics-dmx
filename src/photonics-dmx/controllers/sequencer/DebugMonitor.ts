...
export class DebugMonitor implements IDebugMonitor {
      ...
      try {
        ...
        if (effectCount === 0 && lightIds === 0) {
          ...
          if (lsmLightIds.length > 0) {
            console.log(
              `NOTICE: LightStateManager has ${lsmLightIds.length} tracked lights but none in LTC`,
            )
            ...
            if (lsmLightIds.length <= 10) {
              ...
              console.log(
                `Light IDs (first 10 of ${lsmLightIds.length}): ${lsmLightIds.slice(0, 10).join(', ')}...`,
              )
            }
          }
        }
  ...
  public printLightLayerTable(): void {
    ...
    this.layerManager.getActiveEffects().forEach((layerMap, layer) => {
      ...
      if (firstEffect) {
        ...
        const waitFor =
          transitionIndex < totalTransitions
    ...
    const effectInfoWidth =
      ...
      3 +
      waitUntilWidth +
      3 // Width of info columns with separators
  ...
  public debugLightLayers(): void {
    ...
    for (const lightId of lightIds) {
      ...
      if (finalState) {
        console.log(
          `  Final State: R:${finalState.red}, G:${finalState.green}, B:${finalState.blue}, I:${finalState.intensity}`,
        )
      ...
      for (const layer of layers) {
        ...
        if (layerState) {
          console.log(
            `    Layer ${layer}: R:${layerState.red}, G:${layerState.green}, B:${layerState.blue}, I:${layerState.intensity}`,
          )
        }
      }
    }
    ...
    this.layerManager.getActiveEffects().forEach((lightMap, layer) => {
      lightMap.forEach((effect, lightId) => {
        console.log(
          `  Layer ${layer}, Light ${lightId}: ${effect.name} (${effect.currentTransitionIndex + 1}/${effect.transitions.length})`,
        )
    ...
    this.layerManager.getEffectQueue().forEach((lightMap, layer) => {
      lightMap.forEach((effect, lightId) => {
        console.log(
          `  Layer ${layer}, Light ${lightId}: ${effect.name} (Persistent: ${effect.isPersistent})`,
        )
      })
    })
  }
