...
export class DmxLightManager {
  ...
  private initializeDmxLights(): void {
    const allDmxLights = [
      ...this.config.frontLights,
      ...this.config.backLights,
      ...this.config.strobeLights,
    ]
   ...
   * @returns Array of TrackedLight
   */
  public getLightsInGroup(group: LocationGroup | LocationGroup[]): TrackedLight[] {
    const groups = Array.isArray(group) ? group : [group]
   ...
   */
  public getLightsByTarget(lights: TrackedLight[], target: LightTarget): TrackedLight[] {
    switch (target) {
      case 'all':
        return [...lights]
      case 'even':
