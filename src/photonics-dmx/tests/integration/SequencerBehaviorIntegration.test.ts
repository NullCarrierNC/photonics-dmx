...
const buildSingleLayerEffect = (lights: Effect['transitions'][number]['lights'], layer: number, color: Effect['transitions'][number]['transform']['color'], duration: number, easing: Effect['transitions'][number]['transform']['easing']): Effect => ({
  id: 'test-effect',
...
describe('Sequencer blending and queueing (real harness)', () => {
  it('blends add vs replace with opacity', () => {
    ...
    harness.sequencer.addEffect(
      'base',
      buildSingleLayerEffect(lights, 0, baseColor, 0, 'linear')
    );
    harness.sequencer.addEffect(
      'overlay',
      buildSingleLayerEffect(lights, 1, addColor, 0, 'linear')
    );
    harness.advanceBy(1);
  ...
  it('produces different mid-transition values for easing', () => {
    const sampleMidValue = (easing: Effect['transitions'][number]['transform']['easing']): number => {
      const harness = createSequencerHarness({ frontCount: 1, backCount: 0 });
      const lights = harness.lightManager.getLights(['front'], ['all']);
      ...
      harness.sequencer.addEffect(
        'ease',
        buildSingleLayerEffect(lights, 0, color, 100, easing)
      );
  ...
  it('queues effects with the same name on a layer', () => {
    ...
    const layerManager = (harness.sequencer as unknown as { layerManager: { getEffectQueue: () => Map<number, Map<string, unknown>> } }).layerManager;
