...
describe('Node runtime with real Sequencer', () => {
  ...
  it('executes for-loop with counter updates', () => {
    const eventNode: YargEventNode = {
      id: 'event-1',
      type: 'event',
      eventType: 'beat'
    ...
    const actionNode: ActionNode = {
      ...
      timing: {
        ...
        waitUntilCondition: 'none',
        waitUntilTime: { source: 'literal', value: 0 },
        level: { source: 'variable', name: 'loopIndex', fallback: 0 }
      },
    ...
    const definition: YargNodeCueDefinition = {
      ...
      connections: [
        ...
        { from: 'loop-1', to: 'action-1' }
      ],
      variables: [
        { name: 'loopIndex', type: 'number', scope: 'cue', initialValue: 0 }
      ]
    };
    ...
    const storedValue = cueLevelVarStore.get('loopIndex');
    expect(storedValue?.value).toBe(2);
  });

  it('executes while-loop with max-iteration guard', () => {
    const eventNode: YargEventNode = {
      id: 'event-1',
      type: 'event',
      eventType: 'beat'
    ...
    const loopNode: LogicNode = {
      ...
      comparator: '<',
      left: { source: 'variable', name: 'loopCount' },
      right: { source: 'literal', value: 100 },
      maxIterations: { source: 'literal', value: 3 }
    };

    const incrementNode: LogicNode = {
      id: 'math-1',
      type: 'logic',
      logicType: 'math',
      operator: 'add',
    ...
    const definition: YargNodeCueDefinition = {
      ...
      nodes: {
        events: [eventNode],
        actions: [],
        logic: [initNode, loopNode, incrementNode],
        eventRaisers: [],
        eventListeners: [],
        effectRaisers: []
      },
      connections: [
        ...
        { from: 'var-init', to: 'while-1' },
        { from: 'while-1', to: 'math-1' }
      ],
      variables: [
        { name: 'loopCount', type: 'number', scope: 'cue', initialValue: 0 }
      ]
    };

    const engine = new NodeExecutionEngine(
      compileCue(definition),
      'test-group:while-test',
      harness.sequencer,
      harness.lightManager,
      cueLevelVarStore,
      groupLevelVarStore,
    ...
    const storedValue = cueLevelVarStore.get('loopCount');
    expect(storedValue?.value).toBe(3);
  });
