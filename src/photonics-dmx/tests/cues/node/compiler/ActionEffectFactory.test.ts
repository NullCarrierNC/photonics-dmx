...
describe('ActionEffectFactory', () => {
  ...
  const lights = [createMockTrackedLight({ id: 'l1', position: 0 }), createMockTrackedLight({ id: 'l2', position: 1 })];
  ...
  it('set-color action produces expected Effect with transitions', () => {
    const action: ActionNode = {
      ...
      color: { name: { source: 'literal', value: 'red' }, brightness: { source: 'literal', value: 'medium' }, blendMode: { source: 'literal', value: 'replace' } },
      timing: createDefaultActionTiming()
    };
    const effect = ActionEffectFactory.buildEffect({
      action,
      lights,
      resolvedTiming: { waitForCondition: 'none', waitForTime: 0, duration: 200, waitUntilCondition: 'none', waitUntilTime: 0 },
      resolvedLayer: 0
    });
  ...
  it('blackout action returns null', () => {
    const action: ActionNode = {
      ...
      effectType: 'blackout',
      target: { groups: { source: 'literal', value: 'front' }, filter: { source: 'literal', value: 'all' } },
      color: { name: { source: 'literal', value: 'black' }, brightness: { source: 'literal', value: 'low' }, blendMode: { source: 'literal', value: 'replace' } },
      timing: createDefaultActionTiming()
    };
  ...
  it('resolveLights maps target groups to TrackedLight array', () => {
    const target = { groups: { source: 'literal', value: 'front' }, filter: { source: 'literal', value: 'all' } };
  ...
  it('resolveLights uses light-array variable when variableResolver provides one', () => {
    ...
    const target = { groups: { source: 'variable', name: 'myLights' }, filter: { source: 'literal', value: 'all' } };
